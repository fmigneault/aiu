...
def test_parser_config_list_both():
    ...
    assert config[1]["title"].raw == "song 2"
    assert isinstance(config[1]["duration"], Duration)
    assert config[2]["duration"] == Duration(minutes=4, seconds=50)
    assert config[2]["track"].raw == "3"
    assert config[2]["title"].raw == "song 3"
    assert isinstance(config[2]["duration"], Duration)
    assert config[2]["duration"] == Duration(minutes=1, seconds=23)
...
def test_parser_config_list_track():
    ...
    assert config[0]["track"].raw == "1"
    assert config[0]["title"].raw == "song 1"
    assert config[0]["duration"] is None
    assert config[1]["track"].raw == "2"
    assert config[1]["title"].raw == "song 2"
    assert config[1]["duration"] is None
    assert config[2]["track"].raw == "3"
    assert config[2]["title"].raw == "song 3"
    assert config[2]["duration"] is None
